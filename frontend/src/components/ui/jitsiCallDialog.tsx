...
import jaasCallingService from '../../services/jaasCallingService';
...
const JitsiCallDialog: React.FC<JitsiCallDialogProps> = ({
  isOpen,
  onClose,
  callData,
  isIncoming = false,
  ...
  const handleAcceptCall = async () => {
    ...
    try {
      const success = await jaasCallingService.acceptCall(callData);
      if (success) {
  ...
  const handleDeclineCall = async () => {
    ...
    setCallStatus('ended');
    await jaasCallingService.declineCall(callData);
    onDecline?.();
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        ...
        {callStatus === 'active' && callData.jwt && (
            ...
            <div ref={jitsiContainerRef} className="w-full h-full">
              <JitsiMeeting
                ...
                configOverwrite={{
                  ...
                  startWithVideoMuted: callData.callType === 'audio' ? true : false,
